...
export default class AppointmentRepository {
    ...
    public async getAppointments(email: string, page: number): Promise<RowDataPacket[]> {
        const sql : string = 'SELECT * FROM `Prenotazione` WHERE MATCH (Utente_Email) AGAINST (? IN NATURAL LANGUAGE MODE) AND Id_Prenotazione >= ? ORDER BY Id_Prenotazione LIMIT 10';
